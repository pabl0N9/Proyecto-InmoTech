...
-- ---------------------------------------------------------------------------------------------------------------------
-- Tabla: Propietarios (rol de Personas)
-- Descripción: Tabla normalizada para metadatos del rol propietario vinculado a Personas
-- ---------------------------------------------------------------------------------------------------------------------
...
-- Índices para consultas frecuentes en Propietarios
...
END
-- Agregar columna propietario al catálogo de Inmuebles (si no existe)
...
-- Agregar columna id_propietario al catálogo de Inmuebles (si no existe)
...
-- Crear restricción FK a Propietarios (si no existe)
IF NOT EXISTS (
    SELECT 1
...
-- Índice para búsquedas por propietario (si no existe)
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = 'IX_Inmuebles_id_propietario'
...
BEGIN
    CREATE TABLE Reportes (
        ...
        id_responsable INT NOT NULL,                       -- Respons. actual (administrativo)
        seguimiento_general TEXT NULL,                     -- Descripción del seguimiento global
        -- Quién lo reporta:
        id_persona_reporta INT NOT NULL,                   -- Propietario o administrativo
        -- Auditoría
...
-- Alinear columna faltante en Reportes para compatibilidad con backend
...
-- Índices para optimizar búsquedas en reportes
CREATE NONCLUSTERED INDEX IX_Reportes_Inmueble
    ON Reportes(id_inmueble);
CREATE NONCLUSTERED INDEX IX_Reportes_Estado
    ON Reportes(estado) INCLUDE (fecha_creacion);
CREATE NONCLUSTERED INDEX IX_Reportes_Responsable
    ON Reportes(id_responsable);
GO
...
-- Índices adicionales para optimizar consultas de seguimientos
CREATE NONCLUSTERED INDEX IX_Rubro_Seguimiento_Rubro
    ON Rubro_Seguimiento(id_rubro, fecha);
CREATE NONCLUSTERED INDEX IX_Reporte_Seguimiento_General_Reporte
    ON Reporte_Seguimiento_General(id_reporte, fecha);
GO
...
-- ---------------------------------------------------------------------------------------------------------------------
-- Seed: Propietario de prueba y asociación con INM-001-TEST
-- ---------------------------------------------------------------------------------------------------------------------
-- 1) Crear Persona de prueba si no existe
IF NOT EXISTS (SELECT 1 FROM dbo.Personas WHERE numero_documento = '900000001')
BEGIN
    INSERT INTO dbo.Personas (tipo_documento, numero_documento, nombre_completo, apellido_completo, correo, telefono, tiene_cuenta)
    VALUES ('CC', '900000001', 'Propietario', 'De Prueba', 'propietario.prueba@inmotech.com', '+57 300 111 2233', 0);
    PRINT '✅ Persona de prueba creada (900000001)';
END
ELSE
BEGIN
    PRINT '⚠️ Persona de prueba ya existe (900000001)';
END

DECLARE @id_persona_prop INT = (SELECT id_persona FROM dbo.Personas WHERE numero_documento = '900000001');

-- 2) Registrar en Propietarios (si no existe)
IF NOT EXISTS (SELECT 1 FROM dbo.Propietarios WHERE id_persona = @id_persona_prop)
BEGIN
    INSERT INTO dbo.Propietarios (id_persona, numero_contrato, estado)
    VALUES (@id_persona_prop, 'TEST-001', 'Activo');
    PRINT '✅ Propietario de prueba registrado en Propietarios';
END
ELSE
BEGIN
    PRINT '⚠️ Propietario de prueba ya existe en Propietarios';
END

DECLARE @id_propietario INT = (SELECT id_propietario FROM dbo.Propietarios WHERE id_persona = @id_persona_prop);

-- 3) Obtener id del inmueble de prueba
DECLARE @id_inmueble_test INT = (SELECT id_inmueble FROM dbo.Inmuebles WHERE registro_inmobiliario = 'INM-001-TEST');

-- 4) Asociar persona ↔ inmueble y actualizar catálogo
IF @id_inmueble_test IS NOT NULL AND @id_persona_prop IS NOT NULL
BEGIN
    -- Asociación histórica en Propiedad_inmueble (propietario actual)
    IF NOT EXISTS (
        SELECT 1 
        FROM dbo.Propiedad_inmueble 
        WHERE id_inmueble = @id_inmueble_test 
          AND id_persona = @id_persona_prop 
          AND estado = 'Activo'
          AND fecha_final IS NULL
    )
    BEGIN
        INSERT INTO dbo.Propiedad_inmueble (id_inmueble, id_persona, fecha_inicio, fecha_final, estado)
        VALUES (@id_inmueble_test, @id_persona_prop, CONVERT(date, GETDATE()), NULL, 'Activo');
        PRINT '✅ Asociación Propiedad_inmueble creada (Propietario de prueba ↔ INM-001-TEST)';
    END
    ELSE
    BEGIN
        PRINT '⚠️ Asociación Propiedad_inmueble ya existe (Propietario de prueba ↔ INM-001-TEST)';
    END

    -- Actualizar FK y nombre visible en Inmuebles
    UPDATE dbo.Inmuebles
      SET id_propietario = @id_propietario,
          propietario = (SELECT CONCAT(nombre_completo, ' ', apellido_completo) FROM dbo.Personas WHERE id_persona = @id_persona_prop)
    WHERE id_inmueble = @id_inmueble_test;

    PRINT '✅ Inmuebles.id_propietario y Inmuebles.propietario actualizados para INM-001-TEST';
END
ELSE
BEGIN
...
-- 5) Asignar rol "Propietario" a la persona de prueba si el rol existe
IF EXISTS (SELECT 1 FROM dbo.Roles WHERE nombre_rol = 'Propietario')
BEGIN
    DECLARE @id_rol_prop INT = (SELECT id_rol FROM dbo.Roles WHERE nombre_rol = 'Propietario');
    IF NOT EXISTS (SELECT 1 FROM dbo.Personas_rol WHERE id_persona = @id_persona_prop AND id_rol = @id_rol_prop)
    BEGIN
        INSERT INTO dbo.Personas_rol (id_persona, id_rol) VALUES (@id_persona_prop, @id_rol_prop);
        PRINT '✅ Rol Propietario asignado a la persona de prueba';
    END
    ELSE
    BEGIN
        PRINT '⚠️ La persona de prueba ya tiene rol Propietario';
    END
END
ELSE
BEGIN
    PRINT '⚠️ Rol "Propietario" no existe en Roles; omitiendo asignación';
END
GO
