...
const AdministrativoStatusSelector = ({
  value,
  onChange,
  disabled = false,
  loading = false,
  ...
  // Configuración de estados para administrativos
  const statusConfig = {
    'Activo': {
      label: 'Activo',
      icon: CheckCircle,
  ...
  const currentStatus = statusConfig[value] || statusConfig['Activo'];
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (isOpen && selectRef.current && !selectRef.current.contains(event.target)) {
        ...
        const dropdownElement = document.querySelector('[data-administrativo-status-selector-dropdown]');
      ...
      {isOpen && createPortal(
        <AnimatePresence>
          <motion.div
            ...
            data-administrativo-status-selector-dropdown="true"
            style={{
...
export default AdministrativoStatusSelector;
