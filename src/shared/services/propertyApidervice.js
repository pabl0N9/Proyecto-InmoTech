...
const extractImageSource = (item) => {
  ...
  if (typeof item === 'object') {
    ...
    const cleaned = cleanText(matched, '');
    if (cleaned.startsWith('/uploads')) {
      const baseOrigin = (API_CONFIG.BASE_URL || '').replace(/\/api\/v1$/, '');
...
export const mapInmuebleFromApi = (inmueble = {}) => {
  ...
  const areaConstruida = toNumber(inmueble.area_construida ?? inmueble.areaConstruida ?? inmueble.area_total);
  const areaTerreno = toNumber(inmueble.area_terreno ?? inmueble.areaTerreno);
...
const mapInmuebleToApi = (payload = {}) => {
  ...
  const sanitizeRegistro = (value) => {
    if (!value) return '';
    // Backend exige solo mayúsculas, números y guiones (mínimo 5 chars)
    const cleaned = String(value).trim().toUpperCase().replace(/\s+/g, '-').replace(/[^0-9A-Z-]/g, '');
    return cleaned;
  };

  const body = {
    registro_inmobiliario: sanitizeRegistro(payload.registro_inmobiliario || payload.registro),
...
export const inmueblesAPI = {
  async getPublicInmuebles(filters = {}) {
    try {
      const response = await apiClient.get('/inmuebles/buscar', {
        params: {
          pagina: filters.pagina || 1,
          limite: Math.min(filters.limite || 50, 100),
          _ts: Date.now(),
          ...filters
        }
      });
      ...
      const items = Array.isArray(data.inmuebles)
        ? data.inmuebles.map(mapInmuebleFromApi)
        : Array.isArray(data)
          ? data.map(mapInmuebleFromApi)
          : [];

      return {
        items,
        pagination: normalizePagination(data.paginacion || {}, 1, DEFAULT_LIMIT)
      };
    } catch (error) {
  ...
  async getPublicInmueble(id) {
    try {
      const response = await apiClient.get(`/inmuebles/public/${id}`);
      return mapInmuebleFromApi(extractPayload(response));
    } catch (error) {
      console.error('Error en getPublicInmueble:', error);
      throw error;
    }
  },
  ...
  async getInmuebles(page = 1, limit = DEFAULT_LIMIT, filters = {}) {
    try {
      ...
      return {
        items,
        pagination: normalizePagination(data.paginacion, page, limit)
      };
    } catch (error) {
  ...
  async getInmuebleByRegistro(registro) {
    try {
      const encoded = encodeURIComponent(registro.trim());
      const response = await apiClient.get(`/inmuebles/por-registro/${encoded}`);
      return mapInmuebleFromApi(extractPayload(response));
    } catch (error) {
