...
class ReportesInmobiliariosService {
  ...
  async listarReportes(params) {
    ...
    try {
      ...
      const mapped = reports.map(r => {
        const propietarios = r.inmueble?.propietarios || [];
        const propietarioActivo = propietarios.find(p => p.estado === 'Activo' && p.es_propietario_actual !== false) || propietarios[0];
        const propietarioNombre = propietarioActivo?.propietario?.nombre_completo || '';
        ...
        return {
          ...
          inmueble_propietario: propietarioNombre,
          reporta_nombre: r.reportadoPor?.nombre_completo,
          responsable_nombre: r.reportadoPor?.nombre_completo
        };
      });
  ...
  async obtenerReporte(id) {
    try {
      ...
      const data = reporte.toJSON ? reporte.toJSON() : reporte;
      const propietarios = data.inmueble?.propietarios || [];
      const propietarioActivo = propietarios.find(p => p.estado === 'Activo' && p.es_propietario_actual !== false) || propietarios[0];
      data.inmueble_propietario = propietarioActivo?.propietario?.nombre_completo || '';
      data.responsable_nombre = data.reportadoPor?.nombre_completo || '';
